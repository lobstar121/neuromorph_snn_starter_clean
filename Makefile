...
.PHONY: all test golden hw swq14 compare smoke clean veryclean smoke_compare report alpha_sweep vth_sweep grid_sweep release compare_golden analyze finalize selfcheck ci learn_smoke learn_clean learn_rtl parity
...
> @cp -f fixedpoint_replay.py compare_spikes.py compare_weights.py csv2mem.py sw_q14_from_csv.py analyze_sweeps.py release/
...
>        +OUT=$(ART)/spikes_hw_learn.csv \
>        +WOUT=$(ART)/weights_learned_rtl.hex \
>        +STDP=1

parity:
> python compare_weights.py $(ART)/weights_learned_sw.hex $(ART)/weights_learned_rtl.hex
> python compare_spikes.py $(ART)/spikes_hw_learn.csv $(ART)/spikes_sw_learn_ref.csv
